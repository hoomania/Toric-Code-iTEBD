...
class iTEBD:
    ...
    def suzuki_trotter(
        ...
        # direct hamiltonian
        hamil_shape = self.hamil['d'].shape
        reshape_dim = hamil_shape[0] * hamil_shape[1]
        power = -delta * np.reshape(self.hamil['d'], [reshape_dim, reshape_dim])
        direct = expm(power).reshape(hamil_shape)

        # change side hamiltonian (reverse)
        hamil_shape = self.hamil['r'].shape
        reshape_dim = hamil_shape[0] * hamil_shape[1]
        power = -delta * np.reshape(self.hamil['r'], [reshape_dim, reshape_dim])
        reverse = expm(power).reshape(hamil_shape)
    ...
    def expectation_energy(
        ...
        for i in range(2):
            ...
            expectation_value[i] = ncon(
                [contraction, self.hamil['d'], np.conj(contraction)],
    ...
    def iTEBD_diagram(
        ...
        exp_tensor = self.suzuki_trotter(self.delta)
